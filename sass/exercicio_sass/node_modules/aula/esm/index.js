    ...
    tunnel.send = (func, data) => {
      ...
      if (data.body && ["POST", "PUT", "PATCH"].indexOf(extra.method) !== -1) {
        if (data.body instanceof ref_FormData) {
          ...
          data.body &&
          data.body.constructor &&
          data.body.constructor.name === "ReadStream"
        ) {
          extra.body = data.body;
          //extra.headers["Content-Type"] = "application/octet-stream";
        } else {
      ...
      return ref_fetch(url, extra)
        .then(function(response) {
          let cp = response.headers.get("content-disposition");
